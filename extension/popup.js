...
function displayResult(result) {
  ...
  // Set reasoning for TTS
  currentReasoning = result.reasoning || result.classification;
}

// ── Text-to-Speech (TTS) Logic ──
let currentReasoning = "";
const btnSpeakClaim = document.getElementById("btn-speak-claim");
if (btnSpeakClaim) {
  btnSpeakClaim.addEventListener("click", async () => {
    if (!currentReasoning) return;
    
    btnSpeakClaim.innerHTML = '<span style="font-size: 11px;">⏳</span>';
    btnSpeakClaim.disabled = true;

    const formData = new FormData();
    formData.append("text", currentReasoning);
    ...
    try {
      ...
      audio.onended = () => {
        btnSpeakClaim.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path><path d="M19.07 4.93a10 10 0 0 1 0 14.14"></path></svg>';
        btnSpeakClaim.disabled = false;
      };
      
      audio.play();
      btnSpeakClaim.innerHTML = '<span style="font-size: 10px;">🔊</span>';
    } catch (err) {
      console.error(err);
      btnSpeakClaim.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"></polygon><path d="M15.54 8.46a5 5 0 0 1 0 7.07"></path><path d="M19.07 4.93a10 10 0 0 1 0 14.14"></path></svg>';
      btnSpeakClaim.disabled = false;
